// Preview batch 14 — solmanoktay@gmail.com'a tüm mailler tek email olarak gönderilir
...
const mails = JSON.parse(readFileSync("scripts/leads-batch-14.json", "utf8"))
...
const res = await fetch("https://api.brevo.com/v3/smtp/email", {
  ...
  body: JSON.stringify({
    ...
    subject: `PREVIEW — Batch 14: NL+UK+BE+LU (${mails.length} mail)`,
    htmlContent: `<div style="max-width:720px;margin:0 auto;font-family:system-ui,sans-serif">
      <div style="background:#1a1a1a;color:#fff;padding:16px 24px;border-radius:8px;margin-bottom:24px">
        <strong>BATCH 14 PREVIEW — ${mails.length} mail</strong><br>
        <span style="color:#aaa;font-size:13px">NL (15) · BE (8) · LU (4) · UK yeni şehirler (6) · bnlx-whitelabel-2</span>
      </div>${html}</div>`
  })
})
