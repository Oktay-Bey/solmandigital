...
const leads = JSON.parse(readFileSync("scripts/leads-batch-10.json", "utf8"))
...
const res = await fetch("https://api.brevo.com/v3/smtp/email", {
  ...
  body: JSON.stringify({
    ...
    subject: `PREVIEW — Batch 10: US/CA (${leads.length} mail) — White Label Partnership`,
    htmlContent: `<div style="max-width:720px;margin:0 auto;font-family:system-ui,sans-serif">
      <div style="background:#1a1a1a;color:#fff;padding:16px 24px;border-radius:8px;margin-bottom:24px">
        <strong>BATCH 10 PREVIEW</strong><br>
        <span style="color:#aaa;font-size:13px">${leads.length} mail | US(20) · CA(12)</span>
      </div>${html}</div>`
  })
})
