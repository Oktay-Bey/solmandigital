...
const leads = JSON.parse(readFileSync("scripts/leads-batch-21.json", "utf8").replace(/^﻿/, ""))
...
const res = await fetch("https://api.brevo.com/v3/smtp/email", {
  ...
  body: JSON.stringify({
    ...
    subject: `PREVIEW — Batch 21: İstanbul Yerel Diş Klinikleri (${leads.length} mail)`,
    htmlContent: `<div style="max-width:720px;margin:0 auto;font-family:system-ui,sans-serif">
      <div style="background:#1a1a1a;color:#fff;padding:16px 24px;border-radius:8px;margin-bottom:24px">
        <strong>BATCH 21 PREVIEW — Onaylarsan gönderiyorum</strong><br>
        <span style="color:#aaa;font-size:13px">${leads.length} mail | İstanbul yerel/butik diş klinikleri (fayda odaklı şablon)</span><br>
        <span style="color:#86efac;font-size:13px">Kaynak: find-emails.mjs — sitelerden kazınmış, dedupe + suppress temiz</span>
      </div>${html}</div>`
  })
})
