...
export const rehberPosts: RehberPost[] = [
  ...
  // ─── MARKETPLACE API REHBERLERİ ───────────────────────────────────────────
  {
    ...
    sections: [
      ...
      {
        heading: "Etsy API Key Nasıl Alınır? (Developer Hesabı)",
        body: "Etsy API'yi kullanmadan önce bir Etsy Developer hesabı açıp uygulama (app) oluşturmanız ve API anahtarınızı (Keystring) almanız gerekir. Bu işlem ücretsizdir ve mevcut Etsy hesabınızla yapılır; ayrı bir geliştirici hesabına gerek yoktur. Aşağıdaki adımlar tüm yeni entegrasyonlar için geçerlidir.",
        list: [
          "1. etsy.com/developers/register adresine Etsy hesabınızla giriş yapın (satıcı hesabı şart değil, alıcı hesabı da olur)",
          "2. 'Create a New App' ile uygulamanızı oluşturun: uygulama adı, kullanım amacı ve hangi API'leri kullanacağınızı belirtin",
          "3. Onay sonrası iki anahtar verilir: Keystring (API Key — public, x-api-key header'ında gönderilir) ve Shared Secret (gizli, OAuth imzalamada kullanılır)",
          "4. Shared Secret'i ASLA client tarafına koymayın; yalnızca sunucu ortam değişkeninde (.env) saklayın",
          "5. Onay süresi: basit kullanım için anında, ticari/yüksek hacimli kullanım (Advanced rate limit) için 1–3 iş günü inceleme",
          "6. Personal Access (sadece kendi mağazanız) için ek başvuruya gerek yoktur; üçüncü taraf mağazalara erişim için commercial app onayı gerekir",
        ],
      },
      {
        heading: "OAuth 2.0 Kurulumu",
        body: "API anahtarınızı (Keystring) aldıktan sonra, kullanıcı adına işlem yapabilmek için OAuth 2.0 ile access token üretmeniz gerekir. Etsy v3, PKCE destekli Authorization Code Flow kullanır.",
        list: [
          "1. Keystring + Shared Secret hazır olmalı (bir önceki adım)",
          "2. PKCE destekli Authorization Code Flow ile kullanıcı token'ı üretin",
          "3. Access Token (1 saat) + Refresh Token süresiz → yenileme mekanizması zorunlu",
          "4. Scopes: listings_r, listings_w, transactions_r, shops_r seçin",
          "5. Redirect URI'yi uygulamanıza kaydedin (localhost test için geçerli)",
        ],
      },
      {
