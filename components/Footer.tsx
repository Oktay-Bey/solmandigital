...
export default function Footer() {
  ...
  const linkCls = "text-[0.825rem] text-ink-500 transition-colors hover:text-ondark-muted"
  const headingCls = "mb-3.5 text-[0.65rem] font-bold uppercase tracking-wider text-ink-700"
    ...
    <footer className="border-t border-dark-300 bg-dark-500">
      <div className="mx-auto grid max-w-[1200px] grid-cols-[repeat(auto-fit,minmax(180px,1fr))] gap-10 px-6 py-14">
        ...
        <div>
          ...
          <p className="mb-5 text-[0.8rem] leading-relaxed text-ink-600">
            Web, AI ve otomasyon alanında özel yazılım geliştiren yazılım ofisi.
          </p>
          <div className="flex flex-col gap-2">
            <a
              href={`mailto:${siteConfig.email}`}
              className="flex items-center gap-2 text-[0.8rem] text-ink-500 transition-colors hover:text-ondark-muted"
            >
